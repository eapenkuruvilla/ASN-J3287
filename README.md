...
| `downloadFiles/<HashedId8>.s` | Private key reconstruction value `r` (raw 32-byte big-endian scalar). Final signing key = `(r + e × kU) mod n` where `e = SHA-256(SHA-256(COER(TBS)) ∥ SHA-256(issuer_cert))`, `kU = sk_base` for RSU (no butterfly), and `k_base` comes from HSM (referenced by `dwnl_sgn.priv`). Cannot be reconstructed locally without HSM access for ISS-provisioned bundles. |
...
|---|-------|----------|----------------|
| 1 | **`--certs-dir` local ECQV signing fails for ISS-provisioned RSU bundles** — `dwnl_sgn.priv` is an HSM slot reference in ISS-provisioned RSU bundles, not a raw P-256 scalar, so the local key reconstruction produces an incorrect key. Use `--sign-api-key` (ISS virtual device API) instead. **Pseudonym bundles** (`download/` layout with a raw `dwnl_sgn.priv` scalar) are fully supported locally including butterfly expansion. | High (RSU bundles only) | `out_signed.coer`, `out_ste.coer` |
