...
Decodes a J2735 `MessageFrame` to JSON on stdout. Accepts a UPER hex string, a single COER file, or a directory of COER files. When given a COER file the script decodes the `Ieee1609Dot2Data` wrapper and extracts the J2735 BSM from the `unsecuredData` field automatically.
...
```bash
# From a COER file generated by test_faultybsm.py / create_faultybsm.py
