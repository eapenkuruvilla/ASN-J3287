...
**Example — signed with local ECQV key reconstruction (RSU bundle):**

```bash
python3 create_mbr.py \
    --bsm coer/bad_accel.coer \
    --certs-dir certs/application/e0c324c643aca860 \
    --out-dir coer/
```

The currently valid RSU certificate is selected automatically from `rsu-*/downloadFiles/`.

**Example — signed with local ECQV + butterfly expansion (pseudonym/OBU bundle):**

```bash
python3 create_mbr.py \
    --bsm coer/bad_accel.coer \
    --certs-dir certs/pseudonym/9b09e9e5e5c99a9e \
    --out-dir coer/
```
...
| Local ECQV — RSU bundle | `--certs-dir certs/application/e0c324c643aca860` | `rsu-*/downloadFiles/` (raw scalar) | ✅ `success` |
| Local ECQV — pseudonym bundle | `--certs-dir certs/pseudonym/9b09e9e5e5c99a9e` | `download/{i}/{i}_{j}.cert` + butterfly | ✅ `success` |

All certificates under `certs/` were downloaded from an SCMS provider (ISS) and unzipped. MBRs are generated by RSUs using application certificates stored under `certs/application/<OrgId>/<rsu-N>/`.
...
SaeSol's RA does not expose the `ma-certificate` endpoint. The MA certificate was instead obtained as a decoded-JER JSON file emailed by SaeSol and is stored at `certs/ma_keys/saesol_ma_bublic_key.json`. Use `encode_cert_json.py` to convert it to COER (see below).
...
Output:
```
Output:           certs/ma_keys/saesol_ma_public_key.cert  (202 bytes)
...
│   │   └── <OrgId>/        One directory per organisation (HashedId8 of enrollment cert)
...
│   │   └── saesol_ma_bublic_key.json  SaeSol MA certificate (source JSON from provider)
│   └── pseudonym/          OBU pseudonym certificate bundles (supported by --certs-dir)
