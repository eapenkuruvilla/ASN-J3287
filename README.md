...
```
faulty-bsm-generator/src/create_faultybsm.py
...
```bash
python3 faulty-bsm-generator/src/create_faultybsm.py \
...
Decodes a J2735 `MessageFrame` to JSON on stdout. Accepts a UPER hex string, a single COER file, or a directory of COER files. When given a COER file the script first prints the IEEE 1609.2 signing envelope (`headerInfo`, `signer`, `signature`) and then the decoded J2735 `MessageFrame`.

```bash
python3 decode_j2735.py <hex>          # raw UPER hex string (spaces ignored)
python3 decode_j2735.py <file.coer>    # single Ieee1609Dot2Data COER file
...
```bash
# From a COER file generated by create_faultybsm.py
python3 decode_j2735.py coer/encoded_out_3

# All files in a directory
python3 decode_j2735.py coer/

# From a raw hex string (e.g. copied from decode_mbr.py output)
...
| 3 | **BSM evidence has `signer: digest`** — `create_faultybsm.py` output BSMs carry `signer: digest` (HashedId8 only). The MA needs the full cert to identify and revoke the device. `create_mbr.py` does not substitute the full cert (`signer: certificate`) before embedding the BSM as MBR evidence. | Medium (cert hydration not implemented) | `out_ste.coer` evidence field |
