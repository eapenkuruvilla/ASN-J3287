...
| Local ECQV — RSU bundle | `--certs-dir certs/e0c324c643aca860` | `rsu-*/downloadFiles/` (raw scalar) | ✅ `success` |
...
| `iss_ma_public_key.cert` | ISS pre-production MA (Misbehavior Authority) certificate (`ma.preprod.v2x.isscms.com`); pass as `--recipient-cert certs/iss_ma_public_key.cert` to produce a standards-compliant signed+encrypted MBR (`out_ste.coer`). Obtained via `curl "https://ra.preprod.v2x.isscms.com/v3/ma-certificate?psid=20"` |
...
downloadFiles/<HashedId8>.cert  Authorization Ticket (AT / application cert, 80 bytes)
                                    signed by ↓
trustedcerts/pca                Pseudonym Certificate Authority
                                    signed by ↓
